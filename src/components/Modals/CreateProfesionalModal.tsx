...
import { createProfesional } from '../../services/profesionals';
import { updateProfesionalById, getProfesionalById } from '../../services/profesionals';
...
export default function CreateProfesionalModal({ isOpen, setIsOpen, refetchProfesionals, profesionalId }: CreateProfesionalProps) {
    ...
    useEffect(() => {
        const fetchProfesionalData = async () => {
            if (profesionalId && isOpen) {
                ...
                try {
                    const response = await getProfesionalById(profesionalId);
                    if (response.statusCode === 200) {
                        setFormData({
                            email: response.data.email,
                            name: response.data.profesionalname,
                            password: '',
                            confirmPassword: ''
                        });
                    } else {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (profesionalId) {
                ...
                response = await updateProfesionalById(profesionalId, updateData);
            } else {
                // Create new profesional
                response = await createProfesional({
                    email: formData.email,
                    profesionalname: formData.name,
